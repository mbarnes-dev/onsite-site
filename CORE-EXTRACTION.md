...
- **Routed via call-site `CORE("fn")` with inline fallback retained (Phase-15 state):** `classifyAgainstScope`/`deriveScope`/`scopeMismatch` (Phase 14), `recurringRadar` (Phase 12), `parseIntake` (Phase 13). Runtime routes to core; the inline body degrades gracefully if the bundle is absent. Converting these to definition-delegation (delete the fallbacks) is the next mechanical increment — out of scope for this cleanup.
